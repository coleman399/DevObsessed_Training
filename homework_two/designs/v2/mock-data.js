...
window.AGP_MOCK = (function () {
  ...
  /* Runtime config — would be served by the backend at sign-in so the
   * UI never hardcodes a model name or app version. */
  const config = {
  ...
  /* Full Teams directory for the picker (what Graph /me/joinedTeams returns). */
  const teamsPool = [
    { teamId: 't-elo',  teamName: 'ELO Platform',     channels: [
      { channelId: 'c-elo-general', name: 'general',         members: 24, lastActivity: '11m' },
      { channelId: 'c-elo-fe',      name: 'dev-frontend',    members: 12, lastActivity: '2h' },
      { channelId: 'c-elo-be',      name: 'dev-backend',     members: 9,  lastActivity: '4h' },
      { channelId: 'c-elo-qa',      name: 'qa',              members: 7,  lastActivity: '1d' },
      { channelId: 'c-elo-design',  name: 'design',          members: 5,  lastActivity: '3h' },
    ] },
    { teamId: 't-eng',  teamName: 'AGP Engineering',  channels: [
      { channelId: 'c-eng-announce', name: 'announce',       members: 84, lastActivity: '4h' },
      { channelId: 'c-eng-architecture', name: 'architecture',members: 32, lastActivity: '1d' },
      { channelId: 'c-eng-hiring',  name: 'hiring',          members: 14, lastActivity: '6h' },
      { channelId: 'c-eng-incidents', name: 'incidents',     members: 22, lastActivity: '2d' },
    ] },
    { teamId: 't-co',   teamName: 'AGP Company',      channels: [
      { channelId: 'c-co-general',  name: 'general',         members: 312, lastActivity: '20m' },
      { channelId: 'c-co-it',       name: 'it-help',         members: 78,  lastActivity: '1h' },
      { channelId: 'c-co-events',   name: 'events',          members: 145, lastActivity: '3d' },
    ] },
  ];

  /* Aggregated notifications for the bell dropdown. */
  const notifications = [
    { id: 'n1', kind: 'mention', from: 'Maya Park',     summary: '@Dillon does the partial-cascade branch in #1147 cover the multi-vehicle case?', source: 'ELO Platform · general', time: '11m', unread: true },
    { id: 'n2', kind: 'pr',      from: 'Aamir Khan',    summary: 'requested your review on “chore: add MSAL boot + Graph token cache”',                source: '!1141 · command-station',  time: '10:02', unread: true },
    { id: 'n3', kind: 'workitem',from: 'Brianna Reyes', summary: 'assigned #24812 to you',                                                              source: 'AGP.ELO.WebPortal',         time: '32m',  unread: true },
    { id: 'n4', kind: 'meeting', from: 'Calendar',      summary: 'Sprint planning starts in 15 minutes',                                                 source: '9:30 AM · Teams',           time: '9:15',  unread: false },
    { id: 'n5', kind: 'mention', from: 'Brandon Walls', summary: '@Dillon can you walk through the agentic tool loop in Friday’s demo?',                source: 'AGP Engineering · announce',time: '4h',    unread: false },
  ];

  /* Quick-jump items for the command palette (⌘K). */
  const commandResults = {
    'pr':       [{ kind: 'pr',       title: '!1147 · fix(load-orders): preserve dirty state on partial cascade', sub: 'AGP.ELO.WebPortal · 1h' },
                 { kind: 'pr',       title: '!1141 · chore: add MSAL boot + Graph token cache',                  sub: 'agp-co/command-station · 6h' }],
    'work':     [{ kind: 'work',     title: '#24812 · Vehicle weight cascade clears dirty state',                  sub: 'Bug · Active · 32m ago' },
                 { kind: 'work',     title: '#24798 · Pin file context to chat thread for codebase Q&A',           sub: 'User Story · Active' }],
    'code':     [{ kind: 'code',     title: 'JwtTokenService.cs',                                                  sub: 'AGP.ELO.WebPortal.Api/Services' },
                 { kind: 'code',     title: 'vehicles-form.adapter.ts',                                            sub: 'WebPortal/src/app/main/load-orders' }],
    'mail':     [{ kind: 'mail',     title: 'Re: PR #1147 — vehicle cascade fix',                                  sub: 'Maya Park · 11:24' }],
    'teams':    [{ kind: 'teams',    title: 'ELO Platform · general',                                              sub: '#mentions · 2 unread' }],
    'calendar': [{ kind: 'calendar', title: 'Sprint planning',                                                     sub: 'Today · 9:30' }],
  };
  ...
  /* Feature branches per repo — used by the New PR modal. */
  const branches = {
    'wp':  ['feature/tool-use-loop', 'feat/msal-boot', 'fix/dirty-state-cascade', 'chore/db-migration'],
    'api': ['feat/anthropic-streaming', 'fix/jwt-expiry', 'chore/ef-migration'],
    'cs':  ['feat/command-palette', 'fix/sse-reconnect', 'chore/token-css'],
    'plg': ['docs/onboarding', 'docs/api-reference'],
  };

  return {
    user, config, workItems, pullRequests, repos, repoTree, emails, events,
    teamsChannels, teamsMentions, conversations, seedThread, cannedReplies,
    teamsPool, notifications, commandResults, branches,
  };
})();
